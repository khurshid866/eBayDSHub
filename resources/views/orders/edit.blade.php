...
@section('title', 'Edit Order #' . $order->ebay_order_number)

@section('content')
<div class="max-w-4xl mx-auto space-y-6" x-data="{
    ebay_order_number: '{{ old('ebay_order_number', $order->ebay_order_number) }}',
    amazon_order_number: '{{ old('amazon_order_number', $order->amazon_order_number) }}',
    customer_price: {{ old('customer_price', $order->customer_price) }},
    ad_fee_charges: {{ old('ad_fee_charges', $order->ad_fee_charges) }},
    supplier_cost: {{ old('supplier_cost', $order->supplier_cost) }},
    ...
    <div class="flex items-center justify-between">
        <div>
            <a href="{{ route('orders.show', $order) }}" class="text-xs font-semibold text-slate-400 hover:text-white transition flex items-center space-x-2">
                <i class="fa-solid fa-arrow-left"></i>
                <span>Back to Order Details</span>
            </a>
            <h1 class="text-xl font-extrabold text-white mt-1">Edit Order #{{ $order->ebay_order_number }}</h1>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('orders.update', $order) }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="date" name="order_date" id="order_date" value="{{ old('order_date', $order->order_date ? $order->order_date->format('Y-m-d') : '') }}" required
                ...
                <div>
                    ...
                    <input type="text" name="ebay_order_number" id="ebay_order_number" x-model="ebay_order_number" @input="formatEbay()" @blur="formatEbay()" required
                ...
                <div>
                    ...
                    <input type="text" name="ebay_tracking_number" id="ebay_tracking_number" value="{{ old('ebay_tracking_number', $order->ebay_tracking_number) }}"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="ad_fee_charges" id="ad_fee_charges" x-model.number="ad_fee_charges"
                           class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 focus:border-blue-500 rounded-xl text-sm text-white">
                ...
                <div>
                    ...
                    <input type="text" name="amazon_order_number" id="amazon_order_number" x-model="amazon_order_number" @input="formatAmazon()" @blur="formatAmazon()" required
                           class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 focus:border-blue-500 rounded-xl text-sm font-mono text-white">
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3" class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 focus:border-blue-500 rounded-xl text-sm text-white">{{ old('notes', $order->notes) }}</textarea>
                @error('notes') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
            </div>

            <!-- Form Actions -->
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-slate-800">
                <a href="{{ route('orders.show', $order) }}" class="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-semibold rounded-xl transition">
                    Cancel
                </a>
